<!--
  Provenance: written on branch `slice-01-vocabulary-move` during the first, superseded attempt at
  slab 09, and carried onto this branch verbatim when that branch was deleted. The attempt's code
  and its 474-line PRD were abandoned — map ticket 13 superseded its package layout, and both
  prototypes contradicted it — but this recon is factual reading of the *current* code rather than a
  plan, and ticket 13 kept it as prior art. Nothing below has been edited.

  One correction, from prototype B: §8 offers `DefaultWebhookMappingStrategy` as "the direct model
  for SET_POST_VISIBILITY: one class, three configured instances". It does not transfer, and
  following it fails the boot — the webhook beans each declare a *different* key, while all three
  post-visibility configurations declare one, so three beans would collide in the fold. It is one
  @Component and three Wiring rows, with the variation in `params`. See the PRD's D14.3.
-->
