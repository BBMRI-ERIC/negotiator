...
- **[13 Prototype the compiled-graph package](issues/13-prototype-the-compiled-graph-package.md)** —
  ...
  **The convergent layout is settled twice over** — three packages with `graph` at the bottom,
  ...
  ticket and both trees), **the branch was deleted whole, `recon-strategies.md` included**,
  once B's tree was confirmed to hold what slab 09 needs — B had audited that recon in full and found
  one thing in it that does not transfer — recon §8's `DefaultWebhookMappingStrategy` model for `SET_POST_VISIBILITY`
  ...
  been **deleted whole.** Additive and needing no decision, so they can
...
- **[04 The global state and event metadata contract](issues/04-global-state-event-metadata-contract.md)**
  — **resolved.** The metadata endpoints — **four, not the ticket's three**; `NegotiationEventMetadataDTO`
  is already string-valued — **stay global, with "global" redefined as the active Definition Version of the
  scope's family**, which `resolveForNegotiation()` / `resolveForResource()` already answer. Metadata is no
  longer *asked* globally though: **every caller holding a Negotiation or a Resource asks through that work
  item, and the Lifecycle reads its Definition Version Pin.** That is a rule, now in `backend/CONTEXT.md`
  under the Pin, and it costs nothing — five of the six `EnumBackedLifecycleCatalog` callers already hold the
  ids at the call site. It is **enforced by naming alone**: a guard test was recommended and declined, so the
  recorded hazard is that a caller resolving the active version instead behaves *identically* until a second
  Definition Version exists, then relabels work in flight, in a different stage and a different slab.
  **ADR 0002 is amended, openly rather than quietly** — `V36.5` adds `event.label` NOT NULL,
  `state.description`, `event.description` and `state.progress_order`. The ADR enumerated State's fields and
  said nothing at all about Event's, while four live endpoints already published a label *and* a description
  for both; **`event` has no label column at all**, so for Events *both* published fields were unsourced.
  Deriving labels is unavailable — it renders `Submitted` where the UI says "Under review". New term
  **Progress Order** (the wire field keeps `ordinal`), which answers ticket 03's routed `ordinal` question
  **with a column**; it is not derivable, since a branching graph has no total order, and
  `EnumBackedLifecycleCatalog.resourceStateOrdinal` turns out to have **no production caller** — slab 07
  assumed the column in a javadoc instead of deciding it. **Exactly two intended deltas**, both chosen: the
  two Event collections lose one member each (`START`, `RETURN_FOR_RESUBMISSION`), and single-item lookup
  becomes **case-insensitive against stored names — not `.toUpperCase()`**, which would break a stage-3
  family naming a State `Ready`. Both State collections stay **byte-identical**, so only the Event fixtures
  move. **The finding worth the most to a later slab:** ADR 0009 omits dropped Events because "an Event
  leaves no data residue" — and its *next paragraph* re-homes `information_requirement.for_event`, an
  unconstrained VARCHAR that stores Event names and is the **single** such place in the schema (checked: the
  audit tables carry `changed_to`, a State, and no Event column). A row naming `RETURN_FOR_RESUBMISSION`
  matches nothing, takes a null FK, then loses the column — so the migration slab owes a pre-flight assert.
  Also filed, not fixed: all three controllers' `.toUpperCase()` on the Event-**firing** path carries the
  same stage-3 defect. `EnumBackedLifecycleCatalog` and its test are **deletions**, and one parity assertion
  is **relaxed rather than carved out** — `assertUnrecognisedStateIsRejected` pins `"No enum constant "`
  while its own javadoc says it pins only the shape. **Sub-question 4 is answered in the negative:**
  Negotiation scope is *not* spared, because `/negotiation-lifecycle/events` loses `START`. Resolving this
  leaves ticket [06](issues/06-migration-rehearsal-data.md) as the last thing still moving work between the
  unsliced stage-1 slabs.

## Not yet specified

- **Stage 1 after the evaluator: the cutover and everything downstream.** Swapping the two lifecycle services onto the Transition Evaluator and deleting Spring Statemachine; the audit `state_id` FK conversion (ADR 0008); Information Requirements as a Built-in Stage (0005) with Audience and Quantifier (0006); the coupling layer — Spawn, Feedback, conclusion (0007); the atomic data-cutover migration (0009). The *content* is fixed by the ADRs; the **slab boundaries and gates are not sliceable yet**, because tickets [03](issues/03-state-event-identity-downstream.md), [04](issues/04-global-state-event-metadata-contract.md) and [06](issues/06-migration-rehearsal-data.md) move work between them — 03 and 04 are now resolved, so only 06 still moves work. Graduates as that resolves. Deliberately not pre-sliced. **Two constraints ticket 02 hands the coupling slab specifically:** `SPAWN_RESOURCE_LIFECYCLES` must **not** begin publishing `ResourceStateChangeEvent` — ticket 01 pinned spawn as announcing nothing, so emitting per-Resource events would add N notifications and N webhook deliveries per approval — and it must publish a `ResourceLifecyclesSpawnedEvent` carrying the contacted representatives, because notification now rides on that rather than on arriving at `IN_PROGRESS`. **And one obligation ticket 03 hands slab 07:** eight State names live as raw string literals in `NetworkStatsRepositoryImpl`'s native SQL and JPQL, so deleting the enums raises **no compile error** for them — the slab needs a deliberate grep, not a green build, and line 216 hits the audit table ADR 0008 converts to an FK, so it breaks again there. **And three obligations slab [08](issues/08-definition-schema-and-entities.md) hands these slabs, each filed with its trigger:** the **coupling** slab must decide how a Resource's pin is written at all — the column is `updatable = false` and the link row already exists at Spawn, so the mapping cannot write it ([the three options](../definition-schema-and-entities/issues/09-pinning-an-existing-resource-link.md); no migration needed for any of them) — and what an unresolvable definition does to a Negotiation approval, since `DefinitionResolver` throws a package-private, unmapped exception and a rolled-back 500 is today's default ([the resolver's three shape decisions](../definition-schema-and-entities/issues/10-definition-resolver-shape-is-a-guess.md)); the **cutover** slab must build the index on both pin columns in the same migration that sets them NOT NULL, which is deliberately absent today because the column is 100% NULL and both tables are among the most-written ([why, and the other triggers](../definition-schema-and-entities/issues/08-pin-column-fk-indexes-deferred.md)). **The first slab to read the definition tables also deletes `DefinitionInertnessGuardTest`**, and that deletion belongs in its diff rather than in a quiet edit to the lists inside it. **And three obligations ticket [04](issues/04-global-state-event-metadata-contract.md) hands these slabs:** the **cutover** slab writes `V36.5` — `event.label` NOT NULL, `state.description`, `event.description`, `state.progress_order` — and ADR 0009's seed must populate every one of them, Progress Order included, or the organization roll-up on the Negotiation page silently picks an arbitrary State; the same slab **deletes `EnumBackedLifecycleCatalog` and its test**, once `LifecycleDefinitions`' fifth method exists and its six callers move; and the **migration** slab adds a **pre-flight assert** over `information_requirement.for_event`, because ADR 0009 omits dropped Events while that column stores Event names — a row naming `RETURN_FOR_RESUBMISSION` matches nothing, takes a null FK and then loses the column to the drop.
- **Stage 2 — configurability.** ADR 0004 fixes the design (direct association wins, Network second, Global Default Family last; conflict rejected at write time in `NetworkService`). Open: whether the `DefinitionResolver` seam stage 1 leaves behind is the right shape — now with the three concrete calls slab 08 took, and what would make each wrong, in [The DefinitionResolver's shape is a guess](../definition-schema-and-entities/issues/10-definition-resolver-shape-is-a-guess.md); the precedence walk adds a Resource parameter to `resolveForResource()`, so it changes a signature and not only a body — and whether the conflict check can be meaningfully built and tested before stage 3 supplies the write paths it guards. Sharpens once stage 1 lands. **Carries one sharp ticket already:** [10 Network KPIs name Resource States a custom family need not have](issues/10-network-kpis-name-resource-states.md), raised by ticket 03 — four network statistics define their metric by naming Resource States, and a family omitting them makes the KPI silently report zero. **And one revisit ticket [04](issues/04-global-state-event-metadata-contract.md) files deliberately:** the four lifecycle metadata endpoints answer from *the active version of the scope's family*, which is total only while one Resource family exists. The moment a second goes active they describe the Global Default Family and nothing more, and whether they become per-family, per-Negotiation, or stay as they are is this stage's call.
- **Stage 3 — admin authoring.** Whether it is API-only or API plus a Vue authoring UI is undecided and is the first thing to settle there. Expects `/prototype` tickets before build tickets: graph authoring is an open design problem, not plumbing. Inherits no frontend test net (see standing decision 5). **Carries one deferred check from ticket 02:** whether publishing a Definition Version that omits a **Well-known State** name is refused, warned about, or ignored. Deliberately fog rather than a ticket — the question is *when* the check runs, and the only moment that can refuse is version activation, whose shape is this stage's first undecided thing. Stage 1 is safe by construction: ADR 0009's seed is a faithful transcription, so all five names are present. **And three fields ticket [04](issues/04-global-state-event-metadata-contract.md) adds to whatever authoring surface this stage builds:** a State's description and Progress Order, and an Event's label and description. Progress Order is the one that cannot be left blank — it is not derivable from the Transitions, and the Negotiation page's organization roll-up reads it.
