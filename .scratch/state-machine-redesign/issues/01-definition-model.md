...
- **Ordering is independent per table/scope.** The `order` columns sequence *within* a scope only. Cross-scope evaluation order (definition-level guards before transition guards; the pipeline `required_authority → built-in requirements check → guards`) is engine pipeline logic from ticket 07, **not** encoded in any `order` column.
