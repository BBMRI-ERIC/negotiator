...
public class NegotiationResourceStateMachineConfig extends
  ...
  @Override
  public void configure(StateMachineStateConfigurer<NegotiationResourceState, NegotiationResourceEvent> states) throws Exception {
    states
            .withStates()
  ...
  @Override
  public void configure(StateMachineTransitionConfigurer<NegotiationResourceState, NegotiationResourceEvent> transitions) throws Exception {
    transitions.withExternal()
            .source(NegotiationResourceState.SUBMITTED).event(NegotiationResourceEvent.CONTACT)
            .target(NegotiationResourceState.REPRESENTATIVE_CONTACTED)
            .and().withExternal()
            .source(NegotiationResourceState.SUBMITTED).event(NegotiationResourceEvent.MARK_AS_UNREACHABLE)
            .target(NegotiationResourceState.REPRESENTATIVE_UNREACHABLE)
            .and().withExternal()
            .source(NegotiationResourceState.REPRESENTATIVE_CONTACTED).event(NegotiationResourceEvent.MARK_AS_CHECKING_AVAILABILITY)
            .target(NegotiationResourceState.CHECKING_AVAILABILITY)
            .and().withExternal()
            .source(NegotiationResourceState.REPRESENTATIVE_CONTACTED).event(NegotiationResourceEvent.MARK_AS_UNAVAILABLE)
            .target(NegotiationResourceState.RESOURCE_UNAVAILABLE)
            .and().withExternal()
            .source(NegotiationResourceState.CHECKING_AVAILABILITY).event(NegotiationResourceEvent.MARK_AS_AVAILABLE)
            .target(NegotiationResourceState.RESOURCE_AVAILABLE)
            .and().withExternal()
            .source(NegotiationResourceState.RESOURCE_AVAILABLE).event(NegotiationResourceEvent.INDICATE_ACCESS_CONDITIONS)
            .target(NegotiationResourceState.ACCESS_CONDITIONS_INDICATED)
            .and().withExternal()
            .source(NegotiationResourceState.ACCESS_CONDITIONS_INDICATED).event(NegotiationResourceEvent.MARK_ACCESS_CONDITIONS_AS_MET)
            .target(NegotiationResourceState.ACCESS_CONDITIONS_MET)
            .and().withExternal()
            .source(NegotiationResourceState.ACCESS_CONDITIONS_MET).event(NegotiationResourceEvent.GRANT_ACCESS_TO_RESOURCE)
