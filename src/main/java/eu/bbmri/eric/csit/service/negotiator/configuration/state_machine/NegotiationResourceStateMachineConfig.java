...
public class NegotiationResourceStateMachineConfig extends
  ...
  public void configure(
    ...
    transitions
        .withExternal()
        .source(NegotiationState.CONTACTED).target(NegotiationState.INTERESTED)
        .event(NegotiationEvent.EXPRESS_INTEREST)
        .and().withExternal()
        .source(NegotiationState.INTERESTED).target(NegotiationState.CONCLUDED)
        .event(NegotiationEvent.CONCLUDE)
        .and().withExternal()
        .source(NegotiationState.INTERESTED).target(NegotiationState.ABANDONED)
