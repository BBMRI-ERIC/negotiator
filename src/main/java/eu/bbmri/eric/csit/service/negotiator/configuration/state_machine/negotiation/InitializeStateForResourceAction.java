...
public class InitializeStateForResourceAction implements Action<String, String> {
    ...
    public void execute(StateContext<String, String> context) {
        ...
        String negotiationId =
                context.getMessage().getHeaders().get("negotiationId", String.class);
        Negotiation negotiation = negotiationRepository.findById(negotiationId).orElse(null);
        if (Objects.nonNull(negotiation)){
            negotiation.getAllResources().forEach(
                    resource -> negotiation.setStateForResource(resource.getSourceId(), NegotiationResourceState.SUBMITTED)
            );
