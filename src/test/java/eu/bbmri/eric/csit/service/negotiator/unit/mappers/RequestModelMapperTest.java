...
public class RequestModelMapperTest {
    ...
    void mapRequestToDTO_Ok() {
        ...
        assertEquals(resource.getSourceId(), this.mapper.map(request, RequestDTO.class)
                .getResources().iterator().next().getId());
    }
  ...
  void mapCreateDTOtoRequest_Ok() {
      ...
      assertEquals(requestCreateDTO.getResources().iterator().next().getId(),
