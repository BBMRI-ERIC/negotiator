...
public class RequestControllerTests {
  ...
  public void testGetAll_Ok() throws Exception {
    RequestCreateDTO request = TestUtils.createRequest(false, true, Set.of(testRequest.getId()));
  ...
  public void testCreate_Unauthorized_whenNoAuth() throws Exception {
    ...
    request = TestUtils.createRequest(false, false, Set.of(testRequest.getId()));
    TestUtils.checkErrorResponse(
        mockMvc,
        HttpMethod.POST,
        request,
  ...
  public void testCreate_NotFound_whenProjectDoesNotExist() throws Exception {
    RequestCreateDTO request = TestUtils.createRequest(false, false, Set.of(testRequest.getId()));
    TestUtils.checkErrorResponse(
        mockMvc,
        HttpMethod.POST,
        request,
  ...
  public void testCreate_BadRequest_whenQuery_IsAlreadyAssignedToAnotherRequest() throws Exception {
    RequestCreateDTO createRequest = TestUtils.createRequest(false, false, Set.of(testRequest.getId()));
    ...
    RequestCreateDTO updateRequest = TestUtils.createRequest(false, false, Set.of(testRequest.getId()));
  ...
  public void testUpdate_BadRequest_whenQueryIsAlreadyAssignedToAnotherRequest() throws Exception {
    // Create the negotiation that has the assigned query
    Negotiation negotiationEntityWithQuery =
        modelMapper.map(TestUtils.createRequest(false, false, null), Negotiation.class);
    ...
    // Negotiation body with updated values and query already assigned
    RequestCreateDTO request =
        TestUtils.createRequest(true, false, Set.of(firstRequest.getId(), secondRequest.getId()));
    String requestBody = TestUtils.jsonFromRequest(request);
    mockMvc
        .perform(
  ...
  public void testUpdate_Ok_whenChangeTitle() throws Exception {
    ...
    Negotiation negotiationEntity =
        modelMapper.map(
            TestUtils.createRequest(false, false, Set.of(testRequest.getId())), Negotiation.class);
    negotiationRepository.save(negotiationEntity);
    ...
    // Negotiation body with updated values
    RequestCreateDTO request = TestUtils.createRequest(true, false, Set.of(testRequest.getId()));
    String requestBody = TestUtils.jsonFromRequest(request);
    mockMvc
        .perform(
            MockMvcRequestBuilders.put("%s/%s".formatted(REQUESTS_ENDPOINT, negotiationEntity.getId()))
                .header("Authorization", "Bearer %s".formatted(CORRECT_TOKEN_VALUE))
                .contentType(MediaType.APPLICATION_JSON)
                .content(requestBody))
